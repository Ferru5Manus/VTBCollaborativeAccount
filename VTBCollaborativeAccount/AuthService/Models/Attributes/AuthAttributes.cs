...
namespace AuthService.Models.Attributes;
...
public class AuthAttributes
{
    ...
    public class IpAttribute : ValidationAttribute
    {
        public override bool IsValid(object? value)
        {
            if (value.ToString().Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 4)
            {
                IPAddress ipAddr;
                if (IPAddress.TryParse(value.ToString(), out ipAddr))
                {
                    return true;
                }
                else
    ...
    public class UserAgentAttribute : ValidationAttribute
    {
        public override bool IsValid(object? value)
        {
            if (value.ToString() == "mobile" || value.ToString() == "pc" || value.ToString() == "webbrowser")
            {
                return true;
            }
